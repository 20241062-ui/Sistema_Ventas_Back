...
export const usuarioModel = {
    ...
    obtenerPorId: async (id) => {
        const [rows] = await db.query("SELECT id_usuario, vchnombre, vchapellido, vchapellidoM, vchcorreo, vchRol, vchpassword, Estado FROM tblusuario WHERE id_usuario = ?", [id]);
        return rows[0];
    },

    guardar: async (datos) => {
        const { id_usuario, vchnombre, vchapellido, vchapellidoM, vchcorreo, vchRol, vchpassword } = datos;

        if (id_usuario) {
            let sql = "UPDATE tblusuario SET vchnombre=?, vchapellido=?, vchapellidoM=?, vchcorreo=?, vchRol=?";
            let params = [vchnombre, vchapellido, vchapellidoM, vchcorreo, vchRol];
            ...
            return await db.query(sql, [vchnombre, vchapellido, vchapellidoM, vchcorreo, vchRol, hashedPass]);
        }
    },
