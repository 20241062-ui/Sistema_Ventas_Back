...
$sql = "SELECT * FROM tblinformacion WHERE intid = ?";
...
if ($resultado->num_rows === 0) {
    echo "<script>alert('Información no encontrada'); window.location='informacionEmpresa.php';</script>";
    exit;
}

$informa = $resultado->fetch_assoc();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($_POST['vchtitulo']) || empty($_POST['vchcontenido'])) {
        ...
        $sql = "UPDATE tblinformacion 
                SET vchtitulo = ?, vchcontenido = ?
                WHERE intid = ?";
        ...
        if ($stmt->execute()) {
            echo "<script>alert('Información actualizada correctamente'); window.location='informacionEmpresa.php';</script>";
            exit;
        } else {
            echo "<script>alert('Error al actualizar la Información de la empresa');</script>";
        }
    }
}
?>
...
<header class="header-admin">
  ...
  <form class="panelP-formularioProductos" action="informacionActualizar.php?id=<?= $informa['intid'] ?>"  method="POST">
    
    <h2 >Registrar nueva informción</h2>

    <label for="vchtitulo">Titulo (Misión o Visión)</label>
    <input type="text" id="vchtitulo" name="vchtitulo" value="<?= htmlspecialchars($informa['vchtitulo']) ?>" required>

    <label for="vchcontenido">Contenido:</label>
    <textarea id="vchcontenido" name="vchcontenido" rows="3"><?= htmlspecialchars($informa['vchcontenido']) ?></textarea>
