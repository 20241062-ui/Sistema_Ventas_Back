...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt->bind_param("ssssiisssss",
        $_POST['vchNombre'], $_POST['vchApellido_Paterno'], $_POST['vchApellido_Materno'],
        $_POST['vchColonia'], $_POST['intNo_ExteriorInterior'], $_POST['vchCodigo_Postal'],
        $_POST['vchCalle'], $_POST['vchTelefono'], $_POST['vchCorreo'], $_POST['vchRazon_Social'],
        $_POST['vchRFC']
    );
...
$sql = "SELECT * FROM tblproveedor WHERE vchRFC=?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("s", $id);
$stmt->execute();
$proveedor = $stmt->get_result()->fetch_assoc();

if (!$proveedor) { echo "Proveedor no encontrado."; exit; }
...
<div class="container">
    <main class="main">
        ...
        <form class="panelP-formularioProductos" action="proveedor_actualizar.php" method="POST">

            <h2>Actualizar proveedor</h2>

            <input type="hidden" name="vchRFC" value="<?= htmlspecialchars($proveedor['vchRFC']) ?>">

            <label for="vchNombre">Nombre:</label>
            <input type="text" id="vchNombre" name="vchNombre"
                   value="<?= htmlspecialchars($proveedor['vchNombre']) ?>"
